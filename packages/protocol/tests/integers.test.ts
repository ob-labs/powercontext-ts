...
describe('safe integer wire policy', () => {
  ...
  it('does not stringify bigint as a JSON number substitute', () => {
    const value = BigInt(JS_MAX_SAFE_INTEGER) + 1n
    expect(() => JSON.stringify(value)).toThrow(TypeError)
  })

  it('detects raw JSON integer tokens that JSON.parse would round', () => {
    const raw = '{"position":9007199254740993}'
    expect(JSON.parse(raw).position).toBe(9007199254740992)
  ...
  it('detects unsafe integral exponent and decimal spellings before JSON.parse rounds them', () => {
    ...
    expect(JSON.parse(exponent).position).toBe(9007199254740992)
