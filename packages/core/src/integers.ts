...
import { PowerContextError } from './errors.js'
...
export class SafeIntegerError extends PowerContextError {
...
function integerFromParts(
  ...
  return scaleDigits(sign, digits, exponent)
}

function scaleDigits(sign: bigint, digits: string, exponent: number): bigint | null {
  const maxLength = String(JS_MAX_SAFE_INTEGER).length
  if (exponent >= 0) {
    const significant = digits.replace(/^0+/, '')
    if (significant.length + exponent > maxLength) {
      return sign * (BigInt(JS_MAX_SAFE_INTEGER) + 1n)
    }
 ...
 * Checking `Number.isSafeInteger` after parse cannot implement ADR 0001.
 */
