...
| Milestone | Workstream | Notes |
| --- | --- | --- |
| M1 `client` | Typed Client transport and 52 methods | First publishable product. Install must stay free of native addons. |
| M2 `sqlite-fts` | Deterministic Core (JCS, NFC, hash, refs, UTF-8) | Canonical bytes must match the Python oracle. |
| M2 `sqlite-fts` | SQLite kernel and schema-version gate | Blocked on Python ADR 0001 / 0002 follow-up. |
| M2 `sqlite-fts` | Memory, FTS, and PreparedContext | Vertical slice after the writer gate. |
| M2 `sqlite-fts` | Subset Server and cross-language HTTP E2E | Closes the `sqlite-fts` profile. |
...
| Milestone | When it opens |
| --- | --- |
| M3 domain runtime | Only after the M2 product-line decision says go |
...
1. GitHub milestones: `M1-client`, `M2-sqlite-fts`, `M3-domain`,
   `M4-full-product`.
